...
GOOS=linux CGO_ENABLED=0 go build -ldflags "-extldflags -static -s" -o build/bin/prometheus-federator cmd/prometheus-federator/main.go && REPO=${REPO} TAG=${TAG} make package
```
